...
export class RelationsController {
  ...
  @ApiBody({ type: AssignPermissionsDto, description: 'The permissions to assign' })
  @ApiOkResponse({
  ...
  async assignPermissionsToRole(
    ...
    try {
      ...
      return await this.relationsService.assignPermissionsToRole(roleId, permissionIds);
    } catch (error) {
  ...
  async updatePermissionsForRole(
    ...
    try {
      ...
      const permissionIds = permissions.map((permission: { id: string }) => permission.id);
      return await this.relationsService.updatePermissionsForRole(roleId, permissionIds);
    } catch (error) {
      throw new HttpException(error.message, HttpStatus.BAD_REQUEST);
    }
  }
  ...
  @ApiParam({ name: 'roleId', description: 'The role ID', required: true })
  @ApiBody({ type: RemovePermissionsDto, description: 'The permissions to remove' })
  @ApiOkResponse({
    status: HttpStatus.OK,
    description: 'The permissions have been successfully removed from the role.',
    type: CreateRoleDto,
  })
  @ApiBadRequestResponse({
    status: 400,
  ...
  async removePermissionsFromRole(
    ...
    try {
      ...
      const permissionIds = permissions.map((permission: { id: string }) => permission.id);
      return await this.relationsService.removePermissionsFromRole(roleId, permissionIds);
    } catch (error) {
      throw new HttpException(error.message, HttpStatus.BAD_REQUEST);
    }
  }
