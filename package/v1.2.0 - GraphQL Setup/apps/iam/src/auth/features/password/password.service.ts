...
export class PasswordService {
  ...
  async generateResetPasswordRequestToken(
    user: UserData,
    JWT_SECRET: string,
  ...
  async generateResetPasswordVerificationLink(user: UserData) {
    const JWT_SECRET: string =
      this.configService.get('PASSWORD_RESET_LINK_SECRET') || 'secret-key';
    const JWT_EXPIRES_IN: string =
      this.configService.get('PASSWORD_RESET_LINK_EXPIRES_IN') || '1m';
    return await this.generateResetPasswordRequestToken(
      user,
      JWT_SECRET,
      JWT_EXPIRES_IN,
    );
  ...
  async returnPasswordResetLinkWithOtp(user: UserData) {
    const verificationToken =
      await this.generateResetPasswordVerificationLink(user);
    const FrontendUrl = this.configService.get<string>(
      'PASSWORD_RESET_LINK_FRONTEND_URL',
    );
    if (!FrontendUrl)
  ...
  async verifyOTP(code: string, token: string, res: Response): Promise<void> {
    let email: string = '';
    try {
      ...
      const storedOtp = await this.otpService.get(
        `${auth_otp_key_prefix_for_redis}${email}`,
      );
      if (storedOtp === code) {
  ...
  async validateToken(token: string): Promise<{ email: string; id: ConfigId }> {
    const JWT_SECRET =
      this.configService.get('PASSWORD_RESET_LINK_SECRET') || 'secret-key';
  ...
  async localVariables(): Promise<void> {
    this.OTP = null;
  ...
  async changePassword(
    ...
    const oldPasswordMatches = await PasswordHash.verifyPassword(
      user.hash,
      oldPassword,
    );
    ...
    const currentPasswordMatches = await PasswordHash.verifyPassword(
      user.hash,
      newPassword,
    );
