...
export class MailConfig {
  ...
  get defaultSenderGmail(): string {
    return (
      this.configService.get<string>('MAIL_SERVICE_DEFAULT_SENDER_GMAIL') || Default_Gmail_EMAIL
    );
  }
  ...
  get senderGmail(): string {
    return this.configService.get<string>('MAIL_SERVICE_SENDER_GMAIL') || Gmail_EMAIL;
  }

  get appSenderPassword(): string {
    return this.configService.get<string>('MAIL_SERVICE_APP_SENDER_PASSWORD') || Gmail_PASSWORD;
  }
  //0Auth2.0 Gmail
  get oauthGmailSenderClientId(): string {
    return this.configService.get<string>('MAIL0AUTH_SERVICE_GMAIL_SENDER_CLIENT_ID') || CLIENT_ID;
  }

  get oauthGmailSenderClientSecret(): string {
    return (
      this.configService.get<string>('MAIL0AUTH_SERVICE_GMAIL_SENDER_CLIENT_SECRET') ||
      CLIENT_SECRET
    );
  }
  ...
  get oauthGmailSenderRedirectUri(): string {
    return (
      this.configService.get<string>('MAIL0AUTH_SERVICE_GMAIL_SENDER_REDIRECT_URI') || REDIRECT_URI
    );
  }

  get oauthGmailSenderRefreshToken(): string {
    return (
      this.configService.get<string>('MAIL0AUTH_SERVICE_GMAIL_SENDER_REFRESH_TOKEN') ||
      REFRESH_TOKEN
    );
  }

  get senderGmailAddress(): string {
    return this.configService.get<string>('MAIL0AUTH_SERVICE_SENDER_GMAIL') || ZeroAuth_Gmail_EMAIL;
  }
}
...
export const REDIRECT_URI: string = 'https://developers.google.com/oauthplayground';
