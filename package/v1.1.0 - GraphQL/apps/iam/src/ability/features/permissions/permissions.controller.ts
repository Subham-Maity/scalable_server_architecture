...
export class PermissionsController {
  ...
  async getPermissionById(@Param() params: UserIdDto) {
    try {
      const permission =
        await this.permissionsService.getPermissionById(params);
      if (!permission) {
        throw new NotFoundException(
          `Permission with ID ${params.id} not found.`,
        );
      }
      return permission;
    } catch (error) {
  ...
  @ApiBody({
    ...
    description: 'The new permission data',
  })
  @ApiOkResponse({
    status: HttpStatus.OK,
  ...
  async updatePermissionById(
    ...
    try {
      return await this.permissionsService.updatePermissionById(
        params,
        updatePermissionDto,
      );
    } catch (error) {
